...
而`_mm256_and_ps`顾名思义，就是`_mm256_mul_ps`中8个单精度浮点数同时逻辑与了。不过我们可以关注一下这些指令的延迟和吞吐，比如说`_mm256_mul_ps`在Intel Intrinsics guide中上说在Icelake架构中，延迟是4（4个周期完成），吞吐量（IPC）为2（一个周期内可以处理两条指令）。不过。
...
这个在Icelake架构下延迟为3，IPC为1，吞吐量要比乘法加法要低一些。
